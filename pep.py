...
from adaptive_schema import DecisionController, EmaFilter, LossSignalTracker

try :
    from adaptive_ml import HybridSenseRuntime
except Exception :
    HybridSenseRuntime = None
...
class pepApp :
    def __init__(self) :
        ...
        self.m_legacyPacingRate = self.m_pacingRate
        self.m_legacyCWnd = self.m_cWnd
        ...
        # part 1: Link sensing module
        self.m_linkSampleInterval = 0.05   # 50ms
        self.m_lastLinkSampleTime = 0.0

        # Sliding window samples
        self.m_bwSamples    = deque(maxlen=40)
        self.m_rttSamples   = deque(maxlen=40)
        self.m_lossSamples  = deque(maxlen=40)
        self.m_queueSamples = deque(maxlen=40)

        # Derived link-state features
        self.m_queueDelay    = 0.0
        self.m_rttGradient   = 0.0
        self.m_queuePressure = 0.0
        self.m_ackGapMs      = 0.0
        self.m_streamcQueueSize = 0.0
        self.m_decoderActive = 0.0

        # Rule-based sensing outputs
        self.m_predBw        = 0.0
        self.m_predLossRate  = 0.0
        self.m_lossType      = "NONE"      # NONE / RANDOM / CONGESTION
        self.m_lossTypeStableCount = 0
        self.m_lossRandomScore = 0.0
        self.m_lossCongestionScore = 0.0
        self.m_lossSignalTracker = LossSignalTracker()

        # Online hybrid sensing
        self.m_senseMode = "rule"
        self.m_hybridRuntime = None
        self.m_hybridReady = False
        self.m_sensedBwRawMbps = 0.0
        self.m_sensedLossRawRate = 0.0
        self.m_sensedBwMbps = 0.0
        self.m_sensedLossRate = 0.0
        self.m_hybridResidualBw = 0.0
        self.m_hybridResidualLossRate = 0.0
        self.m_senseBwEma = EmaFilter(alpha=0.35)
        self.m_senseLossEma = EmaFilter(alpha=0.25)

        # Decision layer
        self.m_decisionController = DecisionController()
        self.m_decisionState = "HOLD"
        self.m_decisionBwRefMbps = 0.0
        self.m_decisionPacingMbps = 0.0
        self.m_controlAppliedPacingMbps = 0.0
        self.m_decisionFecRatio = ExtraRepairRate
        self.m_decisionBrakeSignal = 0
        self.m_controlMode = "legacy"
        self.m_adaptiveWarmupSec = 5.0
        self.m_adaptiveMinBwMbps = 2.0
        self.m_adaptiveTakeover = False
        self.m_adaptiveAnchorMbps = 0.0

        # Burst helper state
        self.m_recentBurst     = False
        self.m_lastBurstDegree = 0

        # Link-state logger
        self.m_linkLogPath = "./link_state.csv"
        self.m_linkLogFp   = None
    ...
    def SetAttribute(self, args) :
        ...
        self.m_senseMode     = args.senseMode
        self.m_controlMode   = args.controlMode
        self.m_linkSampleInterval = max(0.01, args.decisionIntervalMs / 1000.0)
        self.m_senseBwEma.alpha = args.senseBwEmaAlpha
        self.m_senseLossEma.alpha = args.senseLossEmaAlpha
        self.m_adaptiveWarmupSec = max(0.0, args.adaptiveWarmupSec)
        self.m_adaptiveMinBwMbps = max(0.1, args.adaptiveMinBwMbps)
        ...
        if self.m_senseMode == "hybrid_gru" :
            if HybridSenseRuntime is None :
                logging.warning("[Adaptive] Hybrid runtime import failed, fallback to rule sensing.")
                self.m_senseMode = "rule"
            elif not args.hybridCheckpoint :
                logging.warning("[Adaptive] Hybrid checkpoint missing, fallback to rule sensing.")
                self.m_senseMode = "rule"
            else :
                try :
                    self.m_hybridRuntime = HybridSenseRuntime(args.hybridCheckpoint, device=args.hybridDevice)
                    logging.info("[Adaptive] Hybrid runtime loaded from %s." % args.hybridCheckpoint)
                except Exception as e :
                    logging.warning("[Adaptive] Hybrid runtime init failed: %s. Fallback to rule sensing." % str(e))
                    self.m_hybridRuntime = None
                    self.m_senseMode = "rule"

        logging.info("[Adaptive] Sensing mode: %s." % self.m_senseMode)
        logging.info("[Adaptive] Control mode: %s." % self.m_controlMode)
    ...
    def InitLinkLogger(self, filepath=None) :
        if filepath is not None :
            self.m_linkLogPath = filepath

        # avoid duplicate opening
        if self.m_linkLogFp is not None :
            return

        self.m_linkLogFp = open(self.m_linkLogPath, "w")
        self.m_linkLogFp.write(
            "ts,"
            "est_bw,est_bw_max,probe_bw,pred_bw,"
            "rtt,rtt_min,queue_delay,rtt_gradient,"
            "loss_rate,pred_loss_rate,"
            "packets_in_flight,cwnd,queue_pressure,"
            "recent_burst,burst_degree,"
            "ack_gap_ms,streamc_queue_size,decoder_active,"
            "loss_random_score,loss_congestion_score,"
            "sense_bw_raw_mbps,sense_loss_raw_rate,"
            "sense_bw_mbps,sense_loss_rate,"
            "adaptive_takeover,adaptive_anchor_mbps,"
            "decision_state,decision_bw_ref_mbps,decision_pacing_mbps,control_applied_pacing_mbps,decision_fec_ratio,decision_brake_signal,"
            "control_mode,"
            "loss_type\n"
        )
        self.m_linkLogFp.flush()

    def BwToMbps(self, bw_pkt_per_sec) :
        return float(bw_pkt_per_sec) * ScPacketSize * 8.0 / 1e6

    def MbpsToBytesPerSec(self, bw_mbps) :
        return max(1.0, float(bw_mbps) * 1e6 / 8.0)

    def BytesPerSecToMbps(self, byte_rate) :
        return max(0.0, float(byte_rate) * 8.0 / 1e6)

    def GetControlRtt(self) :
        if self.m_rttMin > 0 and self.m_rttMin < 10 :
            return self.m_rttMin
        if self.m_rtt > 0 and self.m_rtt < 10 :
            return self.m_rtt
        return 0.6

    def GetAdaptiveAnchorMbps(self) :
        return max(
            self.BwToMbps(self.m_estBw),
            self.BwToMbps(self.m_estBwMax),
            self.BwToMbps(self.m_probeBw),
            self.BwToMbps(self.m_predBw),
            float(self.m_sensedBwRawMbps),
        )

    def CanAdaptiveTakeover(self) :
        self.m_adaptiveAnchorMbps = self.GetAdaptiveAnchorMbps()
        if self.m_controlMode != "adaptive" :
            return False
        if not self.m_peerOnline :
            return False
        if self.m_firstAckTime < 0 :
            return False
        if time.time() - self.m_firstAckTime < self.m_adaptiveWarmupSec :
            return False
        if self.m_senseMode == "hybrid_gru" and self.m_hybridRuntime is not None and not self.m_hybridReady :
            return False
        if self.m_adaptiveAnchorMbps < self.m_adaptiveMinBwMbps :
            return False
        return True

    def ApplyAdaptiveControl(self) :
        if self.m_controlMode != "adaptive" :
            self.m_adaptiveTakeover = False
            return
        self.m_adaptiveTakeover = self.CanAdaptiveTakeover()
        if not self.m_adaptiveTakeover :
            return
        if self.m_decisionPacingMbps <= 0 :
            return

        legacy_pacing_mbps = max(0.1, self.BytesPerSecToMbps(self.m_legacyPacingRate))
        decision_pacing_mbps = max(0.1, float(self.m_decisionPacingMbps))

        if self.m_decisionState == "BRAKE" :
            applied_pacing_mbps = min(
                legacy_pacing_mbps,
                max(decision_pacing_mbps, 0.70 * legacy_pacing_mbps),
            )
        elif self.m_decisionState == "ACCEL" :
            applied_pacing_mbps = max(decision_pacing_mbps, legacy_pacing_mbps)
        else :
            applied_pacing_mbps = max(decision_pacing_mbps, 0.95 * legacy_pacing_mbps)

        self.m_pacing = True
        self.m_pacingRate = self.MbpsToBytesPerSec(applied_pacing_mbps)
        self.m_controlAppliedPacingMbps = applied_pacing_mbps

        decision_pps = applied_pacing_mbps * 1e6 / 8.0 / ScPacketSize
        decision_cwnd = max(10.0, decision_pps * self.GetControlRtt() * CwndGain)

        if self.m_decisionState == "BRAKE" :
            self.m_cWnd = math.floor(max(10.0, min(float(self.m_legacyCWnd), decision_cwnd)))
        else :
            self.m_cWnd = math.floor(max(float(self.m_legacyCWnd), decision_cwnd))

    def GetTargetRepairFreq(self) :
        legacy_repair_freq = max(0.0, self.m_lossRate + ExtraRepairRate)
        if self.m_controlMode != "adaptive" or not self.m_adaptiveTakeover :
            return legacy_repair_freq
        decision_repair_freq = max(0.0, self.m_decisionFecRatio)
        return max(legacy_repair_freq, decision_repair_freq)

    def PredictBandwidthRule(self) :
        bw_ack = self.m_estBw if self.m_estBw > 0 else self.m_estBwMax
        bw_probe = self.m_probeBw if self.m_probeBw > 0 else bw_ack

        bw_trend = bw_ack
        if len(self.m_bwSamples) >= 2 :
            last_bw = self.m_bwSamples[-1]
            prev_bw = self.m_bwSamples[-2]
            bw_trend = max(0.0, last_bw + 0.5 * (last_bw - prev_bw))

        target = 0.80 * bw_ack + 0.10 * bw_probe + 0.10 * bw_trend
        prev_pred = self.m_predBw if self.m_predBw > 0 else target
        
        if target < prev_pred:
            pred_bw = 0.70 * target + 0.30 * prev_pred
        else:
            pred_bw = 0.30 * target + 0.70 * prev_pred

        upper = max(bw_ack, bw_probe) * 1.10
        lower = bw_ack * 0.80 if bw_ack > 0 else 1.0
        pred_bw = min(upper, max(lower, pred_bw))
        return pred_bw
    
    def ClassifyLossTypeRule(self) :
        loss_view = self.m_lossSignalTracker.update(
            loss_rate=self.m_lossRate,
            queue_delay=self.m_queueDelay,
            rtt_gradient=self.m_rttGradient,
            queue_pressure=self.m_queuePressure,
            recent_burst=1.0 if self.m_recentBurst else 0.0,
            burst_degree=float(self.m_lastBurstDegree),
            ack_gap_ms=self.m_ackGapMs,
            streamc_queue_size=self.m_streamcQueueSize,
            decoder_active=self.m_decoderActive,
            rtt_min=self.m_rttMin,
        )
        self.m_lossRandomScore = loss_view["loss_random_score"]
        self.m_lossCongestionScore = loss_view["loss_congestion_score"]
        return loss_view["loss_type"]
    
    def UpdateLinkState(self) :
        bw_sample = self.m_estBw if self.m_estBw > 0 else self.m_estBwMax
        rtt_sample = self.m_rtt
        loss_sample = self.m_lossRate
        queue_delay = max(0.0, self.m_rtt - self.m_rttMin)

        if self.m_cWnd > 0 :
            queue_pressure = min(1.5, float(self.m_packetsInFlight) / max(1.0, float(self.m_cWnd)))
        else :
            queue_pressure = 0.0

        self.m_bwSamples.append(bw_sample)
        self.m_rttSamples.append(rtt_sample)
        self.m_lossSamples.append(loss_sample)
        self.m_queueSamples.append(queue_delay)

        if len(self.m_rttSamples) >= 6 :
            arr = list(self.m_rttSamples)
            prev_mean = sum(arr[-6:-3]) / 3.0
            curr_mean = sum(arr[-3:]) / 3.0
            self.m_rttGradient = curr_mean - prev_mean
        else :
            self.m_rttGradient = 0.0

        self.m_queueDelay = queue_delay
        self.m_queuePressure = queue_pressure
        self.m_ackGapMs = 0.0 if self.m_lastAckTime < 0 else max(0.0, (time.time() - self.m_lastAckTime) * 1000.0)
        self.m_streamcQueueSize = max(0.0, float(self.m_currentMaxSourceId - self.m_lastAckedSourceId))
        self.m_decoderActive = 1.0 if (self.m_duplicatedInorder or self.m_dec.contents.active == 1) else 0.0
        self.m_predLossRate = loss_sample
        self.m_recentBurst = (time.time() - self.m_lastBurstTime) < 1.0

        self.m_predBw = self.PredictBandwidthRule()
        self.m_lossType = self.ClassifyLossTypeRule()
        self.m_sensedBwRawMbps = max(0.1, self.BwToMbps(self.m_predBw))
        self.m_sensedLossRawRate = max(0.0, self.m_predLossRate)
        self.m_hybridResidualBw = 0.0
        self.m_hybridResidualLossRate = 0.0

        if self.m_senseMode == "hybrid_gru" and self.m_hybridRuntime is not None :
            feature_map = {
                "est_bw": self.BwToMbps(self.m_estBw),
                "est_bw_max": self.BwToMbps(self.m_estBwMax),
                "probe_bw": self.BwToMbps(self.m_probeBw),
                "rtt": self.m_rtt,
                "rtt_min": self.m_rttMin if self.m_rttMin < 10 else self.GetControlRtt(),
                "queue_delay": self.m_queueDelay,
                "rtt_gradient": self.m_rttGradient,
                "loss_rate": self.m_lossRate,
                "packets_in_flight": float(self.m_packetsInFlight),
                "cwnd": float(self.m_cWnd),
                "queue_pressure": self.m_queuePressure,
                "recent_burst": 1.0 if self.m_recentBurst else 0.0,
                "burst_degree": float(self.m_lastBurstDegree),
                "pred_bw": self.BwToMbps(self.m_predBw),
                "pred_loss_rate": self.m_predLossRate,
                "loss_random_score": self.m_lossRandomScore,
                "loss_congestion_score": self.m_lossCongestionScore,
                "ack_gap_ms": self.m_ackGapMs,
                "streamc_queue_size": self.m_streamcQueueSize,
                "decoder_active": self.m_decoderActive,
            }
            try :
                self.m_hybridRuntime.push(feature_map)
                hybrid_pred = self.m_hybridRuntime.predict()
                self.m_hybridReady = hybrid_pred["ready"]
                self.m_sensedBwRawMbps = max(0.1, hybrid_pred["pred_bw"])
                self.m_sensedLossRawRate = max(0.0, hybrid_pred["pred_loss_rate"])
                self.m_hybridResidualBw = hybrid_pred["residual_bw"]
                self.m_hybridResidualLossRate = hybrid_pred["residual_loss_rate"]
            except Exception as e :
                logging.warning("[Adaptive] Hybrid predict failed: %s. Fallback to rule sensing." % str(e))
                self.m_hybridRuntime = None
                self.m_senseMode = "rule"
                self.m_hybridReady = False

        self.m_sensedBwMbps = max(0.1, self.m_senseBwEma.update(self.m_sensedBwRawMbps))
        self.m_sensedLossRate = max(0.0, self.m_senseLossEma.update(self.m_sensedLossRawRate))

        decision = self.m_decisionController.update(
            sensed_bw_mbps=self.m_sensedBwMbps,
            sensed_loss_rate=self.m_sensedLossRate,
            loss_type=self.m_lossType,
            est_bw_mbps=self.BwToMbps(self.m_estBw),
            probe_bw_mbps=self.BwToMbps(self.m_probeBw),
            queue_delay=self.m_queueDelay,
            rtt_min=self.m_rttMin if self.m_rttMin < 10 else self.GetControlRtt(),
            rtt_gradient=self.m_rttGradient,
            queue_pressure=self.m_queuePressure,
        ...
        self.m_decisionState = decision["decision_state"]
        self.m_decisionBwRefMbps = decision["decision_bw_ref_mbps"]
        self.m_decisionPacingMbps = decision["decision_pacing_mbps"]
        self.m_decisionFecRatio = decision["decision_fec_ratio"]
        self.m_decisionBrakeSignal = decision["decision_brake_signal"]
        self.ApplyAdaptiveControl()

    def LogLinkState(self) :
        if self.m_linkLogFp is None :
            return

        ts = time.time()
        recent_burst = 1 if (ts - self.m_lastBurstTime) < 1.0 else 0

        self.m_linkLogFp.write(
            f"{ts},"
            f"{self.m_estBw},{self.m_estBwMax},{self.m_probeBw},{self.m_predBw},"
            f"{self.m_rtt},{self.m_rttMin},{self.m_queueDelay},{self.m_rttGradient},"
            f"{self.m_lossRate},{self.m_predLossRate},"
            f"{self.m_packetsInFlight},{self.m_cWnd},{self.m_queuePressure},"
            f"{recent_burst},{self.m_lastBurstDegree},"
            f"{self.m_ackGapMs},{self.m_streamcQueueSize},{self.m_decoderActive},"
            f"{self.m_lossRandomScore},{self.m_lossCongestionScore},"
            f"{self.m_sensedBwRawMbps},{self.m_sensedLossRawRate},"
            f"{self.m_sensedBwMbps},{self.m_sensedLossRate},"
            f"{1 if self.m_adaptiveTakeover else 0},{self.m_adaptiveAnchorMbps},"
            f"{self.m_decisionState},{self.m_decisionBwRefMbps},{self.m_decisionPacingMbps},{self.m_controlAppliedPacingMbps},{self.m_decisionFecRatio},{self.m_decisionBrakeSignal},"
            f"{self.m_controlMode},"
            f"{self.m_lossType}\n"
        )
        self.m_linkLogFp.flush()
    ...
    def PeEstimation(self, nTotalLoss, nTotalSent) :
        if nTotalSent <= 0 :
            return
        alpha = 0.9
        new_lossRate = max(0.0, nTotalLoss / nTotalSent)
    ...
    def UpdateCwnd(self) :
        ...
        effective_rtt = self.GetControlRtt()
        ...
        # Update CWND
        if self.m_constBw :
            cWndPreset = self.m_constBw * effective_rtt * cWndGain
        elif self.m_maxAllowedBw :
            cWndPreset = min(self.m_maxAllowedBw, self.m_estBwMax) * effective_rtt * cWndGain
        else :
            cWndPreset = self.m_estBwMax * effective_rtt * cWndGain
        ...
        self.m_legacyPacingRate = self.m_pacingRate
        self.m_legacyCWnd = self.m_cWnd
        self.m_controlAppliedPacingMbps = self.BytesPerSecToMbps(self.m_pacingRate)
        self.ApplyAdaptiveControl()
    ...
    def RecvDataAck(self, pkt) :
        ...
        if self.m_firstAckTime < 0 :
            self.m_firstAckTime = recvAckTime
        ...
        peBase = sourceSentCount + repairSentCount
        peUpdateValid = peBase >= nsource + nrepair and nTotalLoss >= 0
        if latestRecvPktType == PacketInfoType['SOURCE_PACKET'] and latestRecvSourceId < self.m_lastAckedSourceId :
            peUpdateValid = False
        if latestRecvPktType == PacketInfoType['REPAIR_PACKET'] and latestRecvRepairId < self.m_lastAckedRepairId :
            peUpdateValid = False
        if peUpdateValid :
            self.PeEstimation(nTotalLoss, peBase)
    ...
    def CalculateBytesTxTime(self) :
        return ScPacketSize / max(self.m_pacingRate, 1.0)
    ...
    def TimeToSendRepairPacket(self) :
        ...
        if self.m_newDataIdleState :
            ...
            if self.m_numSentRepairAfterIdle < 1 :
                ...
                if self.m_idleCanSendRepairCount == round(1 / max(self.GetTargetRepairFreq(), 1e-6)) :
            ...
            targetRepairFreq = self.GetTargetRepairFreq()
    ...
    def ReceiveAndHandlePepPacket(self) :
        ...
        if pkt.header.mtype == PepPacketType['HANDSHAKE'] :
            ...
            self.m_lastBurstDegree = burstPacketsNumber
    ...
    def Start(self) :
        while True :
            try :
                ...
                # Periodic link-state sampling for smoother visualization
                currentTime = time.time()
                if currentTime - self.m_lastLinkSampleTime >= self.m_linkSampleInterval :
                    self.UpdateLinkState()
                    self.LogLinkState()
                    self.m_lastLinkSampleTime = currentTime
...
if __name__ == "__main__" :
    ...
    parser.add_argument('--senseMode', required=False, type=str, default='hybrid_gru', choices=['rule', 'hybrid_gru'], help="Online sensing mode")
    parser.add_argument('--hybridCheckpoint', required=False, type=str, default=None, help="Hybrid GRU checkpoint path")
    parser.add_argument('--hybridDevice', required=False, type=str, default='cpu', help="Hybrid runtime device")
    parser.add_argument('--controlMode', required=False, type=str, default='legacy', choices=['legacy', 'adaptive'], help="Execution mode: legacy keeps original pacing/FEC, adaptive enables DecisionController outputs")
    parser.add_argument('--decisionIntervalMs', required=False, type=float, default=50.0, help="Decision loop interval in ms")
    parser.add_argument('--senseBwEmaAlpha', required=False, type=float, default=0.35, help="EMA alpha for sensed bandwidth before DecisionController; 1.0 disables smoothing")
    parser.add_argument('--senseLossEmaAlpha', required=False, type=float, default=0.25, help="EMA alpha for sensed loss rate before DecisionController; 1.0 disables smoothing")
    parser.add_argument('--adaptiveWarmupSec', required=False, type=float, default=5.0, help="Hold original pacing/FEC for this many seconds after the first ACK before adaptive takeover")
    parser.add_argument('--adaptiveMinBwMbps', required=False, type=float, default=2.0, help="Minimum estimated bandwidth required before adaptive takeover")
